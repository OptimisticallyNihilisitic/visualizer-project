...
import { useDroppableBucket } from "../hooks/useDroppableBucket";
import { AGG_OPTIONS } from "../constants/pivotConstants";
...
function DropBucket({
  ...
  const { setNodeRef, className } = useDroppableBucket(id);
      ...
      <div className="drop-bucket-items">
        ...
        {items.map((item) => (
          <div key={item} className="selector-chip">
            <span className="selector-chip-label" title={item}>
              {item}
            </span>
            <button
              className="selector-chip-remove"
