...
export const DataPanel: React.FC<DataPanelProps> = ({ rowsRef }) => {
  ...
  const { rowFields, columnFields, valueFields, aggType } = useAppSelector(
    (s) => s.pivot,
  );

  const { expandedRows, expandedCols, page, rowsPerPage } = useAppSelector(
    (s) => s.ui,
  );
  ...
  const toggleRow = (path: string) =>
    dispatch(uiActions.toggleExpandedRow(path));

  const toggleCol = (path: string) =>
    dispatch(uiActions.toggleExpandedCol(path));
    ...
    return (
      <div className="content">
        <div className="empty-state">
          ...
          <p>
            Drag fields into <strong>Rows</strong> and <strong>Values</strong>{" "}
            to build your pivot
          </p>
        </div>
      </div>
        ...
        <div className="pagination-controls">
          <label className="rows-per-page-label">
            ...
            <select
              value={rowsPerPage}
              onChange={(e) =>
                dispatch(uiActions.setRowsPerPage(Number(e.target.value)))
              }
            >
            ...
            <tr>
              ...
              {visibleCols.map((vc, i) => {
                ...
                return (
                  <th
                    key={vc.path + i}
                    className={`col-header-root col-depth-${Math.min(
                      vc.depth,
                      4,
                    )}${isExpanded ? " col-header-expanded" : ""}`}
                  >
                    <div className="th-inner">
                      {hasChildren && (
                        <button
                          className={`expand-col-btn${
                            isExpanded ? " expand-col-btn--open" : ""
                          }`}
            ...
            <tr className="grand-total-row">
              ...
              {visibleCols.map((vc, i) => (
                <td key={i} className="numeric-cell">
                  {fmt(
                    getCacheVal(cache, "__grand__", vc, valueField, aggType),
                    aggType,
                  )}
                </td>
              ))}
              ...
              return (
                <tr key={node.path + index} className={`level-${node.level}`}>
                  ...
                  {visibleCols.map((vc, i) => (
                    <td key={i} className="numeric-cell">
                      {fmt(
                        getCacheVal(cache, node.path, vc, valueField, aggType),
                        aggType,
                      )}
                    </td>
                  ))}
